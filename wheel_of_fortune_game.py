...
class WOFComputerPlayer(WOFHumanPlayer):
    ...
    def getMove(self, category, obscuredPhrase, guessed):
        ...
        guessed_words = self.getPossibleLetters(guessed)
        good_bad_move = self.smartCoinFlip()
        
        if guessed_words == []:
            return 'pass'
       
        if good_bad_move == True:
            for letter in self.SORTED_FREQUENCIES:
                if letter in guessed_words:
                    return letter
        else:
            rand_char_frm_pssbl_char = random.choice(guessed_words)
            return rand_char_frm_pssbl_char
